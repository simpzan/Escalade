...
public class TranslatedProxySocket: ProxySocket {
    ...
    var actualDestinationPort:UInt16 = 0
    init(socket: GCDTCPSocket) {
        actualDestinationPort = socket.socket.connectedPort
...
public class NATProxyServer: GCDProxyServer {
    ...
    override open func handleNewGCDSocket(_ socket: GCDTCPSocket) {
        let proxySocket = TranslatedProxySocket(socket: socket)
